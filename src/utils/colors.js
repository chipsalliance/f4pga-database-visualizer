...
export function findNearestPaletteColor(color) {
    return tileColors.reduce((prev, current) => {
        const distance = chroma.distance(color, current);
        return (distance < prev[0]) ? [distance, current] : prev;
    }, [Infinity, undefined])[1];
}


export function findMostContrastingColor(base, colorsList) {
    return colorsList.reduce((prev, current) => {
        const contrast = chroma.contrast(base, current);
        return (contrast > prev[0]) ? [contrast, current] : prev;
    }, [0, undefined])[1];
}


export function firstColorWithContrast(base, colorsList, minContrast) {
    for (const color of colorsList) {
        if (chroma.contrast(base, color) >= minContrast)
            return color;
    }
    return undefined;
}
...
// Material Colors which have:
// * contrast at least 4.5 with either white or black
// * not too dark nor too light (i.e. in 20-80% range)
const tileColors = materialColors.reduce((a, c) => {
    const lightness = chroma(c).get("lab.l");
    if ((lightness > 20 && lightness < 80) &&
            (chroma.contrast(c, "#fff") >= 4.5) || (chroma.contrast(c, "#000") >= 4.5))
        a.push(c);
    return a;
}, []);
